...
- [x] Fix SRM `boot dka0` device open. The NCR no-data completion path now
  resumes SCRIPTS after the COMMAND move before delivering STATUS/MESSAGE, so
  SRM `disks` no longer reports `file open failed` for the DKA devices.
